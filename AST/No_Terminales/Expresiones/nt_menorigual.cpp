#include "NT_MenorIgual.h"

Resultado *NT_MenorIgual::Interpretar(Environment *ctx,EnvironmentFunc *ctx2) {
    ...
    if ((izqTipo == "Integer" && (derTipo == "Integer" || derTipo == "Float" || derTipo == "Boolean")) || (izqTipo == "Float" && (derTipo == "Float" || derTipo == "Integer" || derTipo == "Boolean")) || (izqTipo == "Boolean" && (derTipo == "Boolean" || derTipo == "Integer" || derTipo == "Float"))) {
        bool lessEqual = izqValor.toDouble() <= derValor.toDouble();
        resultado = new Resultado(lessEqual);
    }
    else {
...
QString NT_MenorIgual::Graficar() {
...
NT_MenorIgual::NT_MenorIgual(AbstractExpr* izquierda, AbstractExpr* derecha) : Izquierda(izquierda), Derecha(derecha) {}
