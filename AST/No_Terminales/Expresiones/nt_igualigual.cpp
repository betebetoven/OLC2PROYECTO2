#include "NT_IgualIgual.h"

Resultado *NT_IgualIgual::Interpretar(Environment *ctx,EnvironmentFunc *ctx2) {
    ...
    if ((izqTipo == "Integer" && (derTipo == "Integer" || derTipo == "Float" || derTipo == "Boolean")) || (izqTipo == "Float" && (derTipo == "Float" || derTipo == "Integer" || derTipo == "Boolean")) || (izqTipo == "Boolean" && (derTipo == "Boolean" || derTipo == "Integer" || derTipo == "Float"))) {
        bool equal = izqValor.toDouble() == derValor.toDouble();
        resultado = new Resultado(equal);
    }
    // String == String
    else if (izqTipo == "String" && derTipo == "String") {
        bool equal = izqValor.toString() == derValor.toString();
        resultado = new Resultado(equal);
    }
    else {
...
QString NT_IgualIgual::Graficar() {
...
NT_IgualIgual::NT_IgualIgual(AbstractExpr* izquierda, AbstractExpr* derecha) : Izquierda(izquierda), Derecha(derecha) {}
